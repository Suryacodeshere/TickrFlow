...
export default function LoginPage() {
  ...
  useEffect(() => {
      ...
      if (clientId && typeof window !== 'undefined' && (window as any).google) {
        try {
          (window as any).google.accounts.id.initialize({
            ...
            callback: (response: any) => {
              if (response && response.credential) {
                // Pass the ID token as the credential
          ...
          (window as any).google.accounts.id.renderButton(
            ...
            { theme: 'filled_black', size: 'large', width: 350, shape: 'pill' }
          );
        } catch (err) {
    ...
    if (typeof window !== 'undefined' && (window as any).google) {
      ...
      (window as any).initializeGoogle = initializeGoogle;
    }
  }, []);
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-slate-950/40">
      <div className="w-full max-w-md space-y-8 glass-panel p-8 rounded-3xl shadow-2xl border border-white/10 relative overflow-hidden">
        ...
        <div className="mt-6 flex justify-center w-full min-h-[44px]">
          ...
          {/* Fallback mock button if Google script is blocked/fails */}
          <button
            type="button"
            onClick={() => setShowGoogleMock(true)}
            id="googleFallbackBtn"
            className="hidden w-full items-center justify-center gap-3 py-3 border border-white/10 bg-slate-900/40 hover:bg-slate-900/80 rounded-xl text-white font-semibold text-sm shadow-md"
          >
            Continue with Mock Google (Testing)
          </button>
        </div>
      ...
      <Script 
        ...
        onLoad={() => {
          if (typeof window !== 'undefined' && (window as any).initializeGoogle) {
            (window as any).initializeGoogle();
          }
        }}
      />
    </div>
